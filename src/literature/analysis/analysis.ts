...
export class Analysis {
	...
	calculateDistance(lineID: string, negative?: boolean) {
		const refLineIndex = findIndex(this.lines, (line) => line.id === '0mm-10mm');
		if (refLineIndex === -1) return undefined;
		const refPixels = this.lines[refLineIndex].distance;
		const targetLineIndex = findIndex(this.lines, (line) => line.id === lineID);
		if (targetLineIndex === -1) return undefined;
